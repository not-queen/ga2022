...
static void load_resources(project_game_t* game);
static void unload_resources(project_game_t* game);
static void spawn_player(project_game_t* game, int index);
static void spawn_camera(project_game_t* game);
static void update_players(project_game_t* game);
static void draw_models(project_game_t* game);
static void spawn_traffic(project_game_t* game, int index);
static void update_traffic(project_game_t* game);

project_game_t* project_game_create(heap_t* heap, fs_t* fs, wm_window_t* window, render_t* render)
{
	project_game_t* game = heap_alloc(heap, sizeof(project_game_t), 8);
	game->heap = heap;
	game->fs = fs;
	game->window = window;
	game->render = render;
	...
	audio_init();
	game->audio1 = read_file("1.mp3");
	game->audio1->volume = 1.0f;

	game->audio2 = read_file("2.wav");
	game->audio2->volume = 5.0f;

	game->audio3 = read_file("3.wav");
	game->audio3->volume = 5.0f;
